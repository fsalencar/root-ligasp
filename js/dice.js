...
// Dado de capanga: cubo 6 faces com estrelas (y=amarelas, o=laranjas)
const CAPANGA_FACES = [
  {y:1,o:1}, {y:1,o:1}, {y:1,o:1},  // faces 0-2: 1 amarela + 1 laranja  (3×)
  {y:2,o:1}, {y:2,o:1},               // faces 3-4: 2 amarelas + 1 laranja (2×)
  {y:2,o:2},                           // face  5  : 2 amarelas + 2 laranjas (1×)
];
...
// ── Textura: número (dado de batalha) ─────────────────────────────────────────
...
// ── Textura: estrelas (dado de capanga) ───────────────────────────────────────
const STAR_POS = {
  2: [[84,84],[172,172]],
...
function makeCapangaFaceTex(yCount, oCount) {
  const c = document.createElement('canvas'); c.width = c.height = 256;
  const ctx = c.getContext('2d');
  ctx.fillStyle = CAPANGA_BODY; ctx.fillRect(0,0,256,256);
  const pos = STAR_POS[yCount + oCount] || [[128,128]];
  pos.slice(0, yCount).forEach(([x,y]) => drawStar4(ctx,x,y,40,13,STAR_Y));
  pos.slice(yCount).forEach(([x,y])    => drawStar4(ctx,x,y,31,10,STAR_O));
  return new THREE.CanvasTexture(c);
}

// ── Geometria dodecaedro (12 grupos + UV por face) ────────────────────────────
...
function makeCapangaDie() {
  const mats = CAPANGA_FACES.map(f =>
    new THREE.MeshStandardMaterial({map:makeCapangaFaceTex(f.y,f.o),roughness:0.35,metalness:0.08})
  );
  ...
  if (rolling) {
    ...
    if (diceMode==='batalha' && dice.length===2) {
      ...
      if(t<0.40){const p=ease(t/0.40);x1=-1.65+1.3*p;x2=1.65-1.3*p;}
        ...
        x1=-0.35;x2=0.35;
        if(!pulsed){pulsed=true;dice.forEach(d=>d.scale.setScalar(1.2));setTimeout(()=>{if(dice.length)dice.forEach(d=>d.scale.setScalar(1));},130);}
      }
    ...
    // posição: capanga (quique)
    if (diceMode==='capanga' && dice.length===1) {
      dice[0].position.y = t<0.35 ? Math.sin(t*Math.PI/0.35)*0.9 : 0;
    }

    // captura quaternion inicial do pouso
    ...
    if (rollT>=1) {
      ...
      if(diceMode==='capanga'&&dice.length===1){dice[0].position.y=0;}
      showResult();
    }
...
function showResult() {
  ...
  if (diceMode==='batalha') {
    ...
    el.innerHTML=`
      <div style="text-align:center;">
        <div style="letter-spacing:5px;line-height:1.2">${yS.repeat(f.y)}${oS.repeat(f.o)}</div>
        <div style="font-size:0.65rem;color:var(--text3);font-family:sans-serif;margin-top:6px">
          <span style="color:${STAR_Y}">${f.y} amarela${f.y>1?'s':''}</span>
          &nbsp;·&nbsp;
          <span style="color:${STAR_O}">${f.o} laranja${f.o>1?'s':''}</span>
        </div>
      </div>`;
  }
}
...
window.rolarDados = function() {
  ...
  if(diceMode==='batalha'){
    ...
    rollResults=[fi];
    targetQuats=[getTargetQuatBox(fi)];
  }
...
window.setDiceMode = function(mode) {
  ...
  if(desc) desc.innerHTML=mode==='batalha'
    ...
    :`<strong style="color:var(--text2)">Capanga:</strong> 3× <span style="color:${STAR_Y}">✦</span><span style="color:${STAR_O}">✦</span> &nbsp;·&nbsp; 2× <span style="color:${STAR_Y}">✦✦</span><span style="color:${STAR_O}">✦</span> &nbsp;·&nbsp; 1× <span style="color:${STAR_Y}">✦✦</span><span style="color:${STAR_O}">✦✦</span>`;
  const colorRow=document.getElementById('diceColorRow');
  if(colorRow) colorRow.style.display=mode==='batalha'?'flex':'none';
  const el=document.getElementById('diceResultBox');
  if(el){el.style.display='none';el.innerHTML='';}
  showingResult=false;
  if(ready) setupDice();
