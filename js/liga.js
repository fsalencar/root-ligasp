  ...
  modal.innerHTML = `
    <div class="modal-box" style="max-width:400px;width:95%;max-height:85vh;display:flex;flex-direction:column;">
      ...
      <div style="border-top:1px solid var(--border);margin-top:10px;padding-top:10px;">
        <button onclick="_abrirFormNovoJogadorParaLiga(${slotIdx})"
...
let _ligaPickerActiveSlot = null;

function _abrirFormNovoJogadorParaLiga(slotIdx) {
  _ligaPickerActiveSlot = slotIdx;
  _abrirFormNovoJogador(null);
  // Redireciona o botão "←" para voltar ao picker da liga
  const modal   = document.getElementById('modalJogadores');
  const backBtn = modal?.querySelector('.modal-header .modal-close-btn');
  if (backBtn) backBtn.onclick = () => _abrirPickerJogadorLiga(slotIdx);
  // Injeta marcador de slot da liga para _confirmarSalvarJogador usar
  const box = modal?.querySelector('.modal-box');
  if (box) {
    const h = document.createElement('input');
    h.type = 'hidden'; h.id = 'fjLigaSlotIdx'; h.value = slotIdx;
    box.appendChild(h);
  }
}
...
async function _checarFotoMod(jwt, url) {
  try {
    ...
    return res.ok ? await res.json() : { ok: true, skipped: true };
